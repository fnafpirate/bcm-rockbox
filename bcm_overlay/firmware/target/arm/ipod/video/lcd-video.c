...
#ifndef BOOTLOADER
static struct mutex bcm_io_mtx;
#define BCM_IO_LOCK()   mutex_lock(&bcm_io_mtx)
#define BCM_IO_UNLOCK() mutex_unlock(&bcm_io_mtx)
#else
#define BCM_IO_LOCK()
#define BCM_IO_UNLOCK()
#endif
...
#ifndef BOOTLOADER
/*** BCM host interface support (bcm_host.c) ***/

/* Take exclusive use of the BCM ports.  Thread context only; must not be called from the tick. */
...
/* Release the BCM ports without kicking an LCD update: a pending update, if any, is picked up by
   the next lcd_tick() because lcd_state.state is left untouched. */
...
/* 32-bit aligned VC addresses and lengths only.  memcpy() keeps unaligned RAM buffers safe on
   the ARM7-class core; the compiler turns it into plain loads/stores when the buffer is aligned. */
void bcm_io_read(unsigned vc_addr, void *dst, unsigned len)
{
    unsigned char *d = (unsigned char *)dst;
    unsigned i;
    for (i = 0; i < len; i += 4)
    {
        unsigned v = bcm_read32(vc_addr + i);
        memcpy(d + i, &v, 4);
    }
}

void bcm_io_write(unsigned vc_addr, const void *src, unsigned len)
{
    const unsigned char *s = (const unsigned char *)src;
    unsigned i;
    if (len == 0)
        return;
    bcm_write_addr(vc_addr);          /* latch once; the destination auto-increments */
    for (i = 0; i < len; i += 4)
    {
        unsigned v;
        memcpy(&v, s + i, 4);
...
void bcm_io_doorbell(void)
{
    BCM_CONTROL = 0x31;
}

void bcm_io_idle(void)
{
    sleep(1);                        /* one tick; bcm_host.c poll counts are in ticks */
}
